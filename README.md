...
- `https://lms.sysu.edu.cn/mod/forum/view.php*`
...
2. 在浮窗下拉中选择模式（默认 `国家安全`，可切到 `心理健康`，会自动记住）
3. 脚本会自动播放视频，并持续监控页面观看进度（`.num-bfjd span`）
4. 播放过程中每秒检测进度：
   - 若进度 `>= 100`：立即跳转到下一活动（不等 `ended`）
   - 若视频 `ended` 且进度 `< 100`：自动从头重播当前视频
5. 心理健康模式下，进入 `mod/forum/view.php`（讨论/公告）会自动跳到下一活动
6. 章节末尾会优先进入本章节小测（若课程结构如此）
7. 你手动完成小测并提交后，进入 `quiz/review` 页面会自动继续下一活动（可配置关闭）
8. 若下一活动是期末考试且 `stopBeforeFinalExam=true`，脚本会停下并提示手动开始
...
- 模式默认是 `国家安全`；切换到 `心理健康` 后，forum（讨论/公告）页面会自动跳过到下一活动
...
先看右下角模式是否为 `心理健康`。如果仍是 `国家安全`，forum 页面不会自动跳转。
